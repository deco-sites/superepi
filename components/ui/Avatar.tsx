...
function Avatar({ content, variant = 'default' }: Props) {
    return (
        <div class='avatar placeholder text-xs font-bold'>
            <div class={`w-8 h-8 ${colors[content] ?? colors[variant]} ${variants[variant]}`}>
                <span class='uppercase'>{colors[content] ? '' : content.substring(0, 2)}</span>
            </div>
        </div>
    )
}
