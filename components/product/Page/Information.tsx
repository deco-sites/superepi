...
export const ProductInfo = ({ product }: Props) => {
  ...
  const description = isVariantOf?.description || product.description ||
    rawProduct.Model.Descriptions[1].Value;
  const { availability } = useOffer(offers);

  const showMetadatas = new Set(["Modelo", "Conteúdo", "Marca", "Referência"]);
  const metadatas = rawProduct.Model.ExtendedMetadatas.map((
    { Name, Value },
  ) => [
    Name,
    Value,
  ...
  const CA = rawProduct.Model.ExtendedMetadatas.find((i) =>
    i.Name === "ConsultaCA"
  )?.Value;
  const video = rawProduct.Model.ExtendedMetadatas.find((i) =>
    i.Name === "Video"
  )?.Value;
    ...
    <div class="flex flex-col gap-5 w-full lg:max-w-[29%]">
      ...
      <ul class="gap-3 grid grid-cols-2 w-full">
        {[...metadatas, ["Marca", product.brand?.name as string]]
          .sort(
            ([a], [b]) =>
              [...showMetadatas].indexOf(a) - [...showMetadatas].indexOf(b),
          )
          .map(([k, v]) => (
      ...
      <div class="flex flex-col gap-2">
        <a
          href="#description"
          class="text-[#151515] text-sm font-medium italic"
        >
