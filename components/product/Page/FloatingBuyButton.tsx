...
export default function ({ product }: Props) {
    ...
    const { discountTicket = 0, billingDuration, billingIncrement } = useOffer(offers)
    const isHidden = useSignal(true)

    useEffect(() => {
        addEventListener('scroll', () => {
            const scrollPercentage = globalThis.scrollY / (document.body.scrollHeight - innerHeight)
