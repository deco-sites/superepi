...
export const Gallery = ({ product }: GalleryProps) => {
  ...
  const smallImages = mediaGroups.map(({ Small: { MediaPath } }) =>
    CDN + MediaPath
  );
  const mediumImages = mediaGroups.map(({ Medium: { MediaPath } }) =>
    CDN + MediaPath
  );
    ...
    <div class="flex flex-col gap-2 w-full max-w-[500px] relative px-6">
      ...
      <Slider.Root id={id}>
        <Slider.Carousel id={id} class="carousel gap-2">
          {[...mediumImages, ...mediumImages, ...mediumImages].map((
            url,
            index,
          ) => (
        ...
        <div class="flex items-center gap-2">
          ...
          <ul class="carousel gap-2 w-full">
            {[...smallImages, ...smallImages, ...smallImages].map((
              url,
              index,
            ) => (
