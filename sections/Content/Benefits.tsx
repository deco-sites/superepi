...
export default function Benefits(props: Props) {
    ...
    const listOfBenefits = benefits.map((benefit, index) => {
        ...
        const benefitLayout =
            !layout?.variation || layout?.variation === 'Simple' ? 'tiled' : 'piledup'

        return (
            <div
                class={`
        ${reverse ? 'bg-primary text-primary-content p-4 lg:py-4' : ''} flex gap-4 ${
            isRow
            ...
            showDivider && benefitLayout !== 'piledup'
                ...
                : ''
        } ${showDivider ? 'lg:border-b-0' : ''} ${showDivider && !reverse ? 'lg:pb-0' : ''}`}
            >
                <div class='flex-none'>
                    <Icon
                        id={benefit.icon}
                ...
                <div class='flex flex-col gap-1 lg:gap-2'>
                    <div
                        class={`text-[10px] leading-7 ${reverse ? 'text-base-100' : 'text-[#000]'}`}
                        dangerouslySetInnerHTML={{ __html: benefit.label || '' }}
                    />
                    <div
